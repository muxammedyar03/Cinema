...
const RU: Record<string, string> = {
	...
	USER_MISSING: "В Telegram нет данных пользователя",
	BOT_TOKEN_UNCONFIGURED: "Авторизация недоступна",
	SELF_REFUND_WINDOW_CLOSED: "До сеанса меньше часа — отмена только в кинотеатре",
...
export function errorText(err: unknown, fallback = "Ошибка запроса"): string {
	if (err instanceof ApiError) {
		...
		if (err.status === 401) return "Нужна авторизация Telegram";
